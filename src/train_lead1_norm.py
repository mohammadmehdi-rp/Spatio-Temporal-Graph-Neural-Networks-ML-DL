#!/usr/bin/env python3
"""
Lead-1 with per-node label normalization (train busy μ/σ).
Predict y_{t+1} from K-step history via GraphSAGE encoder + TCN/GRU head.
"""
...
def compute_node_norm(Y, idx, lead=1):
    Yt=torch.from_numpy(Y[idx])  # [T,N]
    # shift to lead target positions within train window
    # we’ll just compute over all Y at train idx (busy-only)
    m=(Yt>0).float(); cnt=m.sum(0).clamp(min=1.0)
    mu=(Yt*m).sum(0)/cnt
...
def main():
    ...
    ap.add_argument("--temporal", choices=["tcn","gru"], default="gru")
    ...
    ap.add_argument("--out", default="lead1_v2_norm.pt")
    args=ap.parse_args()
    ...
    Fin=X.shape[2]; N=X.shape[1]; K=args.K
    ...
    for ep in range(1, args.epochs+1):
        ...
        for t in train_pos:
            ...
            yhat=model.head(Hseq)
            yn=(yy - mu_node)/sd_node
        ...
        with torch.no_grad():
            ...
            for t in val_pos:
                ...
                pred=model.head(Hseq); yy=Y[t+1]; m=(yy>0)
                if m.any(): yh.append(pred[m]); yt.append(yy[m])
            val_rmse=rmse(torch.cat(yh), torch.cat(yt)) if yh else 1e9
        print(f"[lead1-norm][ep {ep:03d}] train_loss={tot/max(1,steps):.5f} val_busy_RMSE={val_rmse:.3f}")
