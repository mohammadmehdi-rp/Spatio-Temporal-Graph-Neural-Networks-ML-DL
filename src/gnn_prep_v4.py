...
def build_edges(nodes, links_file):
    ...
    for i,n in enumerate(nodes):
        sw,_=parse_sw_port(n); sw_buckets.setdefault(sw, []).append(i)
    src,dst=[],[]
        ...
        with open(links_file) as f:
            for line in f:
                ...
                if len(p)==2 and p[0] in nodes and p[1] in nodes:
                    a=nodes.index(p[0]); b=nodes.index(p[1])
                    src+=[a,b]; dst+=[b,a]
...
def fit_ar1_per_node(Y, train_idx, lam=1e-6):
    """Fit y_{t+1} ≈ a_i * y_t on TRAIN (busy-prioritized). Return a_i per node."""
    T,N = Y.shape
    train = np.array(train_idx, dtype=int)
    t_valid = train[(train>=1) & (train<T-1)]
    a = np.zeros(N, np.float32)
    for i in range(N):
        y = Y[:,i].astype(np.float64)
        x = y[t_valid-1]  # y_t
        z = y[t_valid]    # y_{t+1} (approx – one-step forward in train region)
        # busy-first mask; if none, fall back to all
        m = (x>0) | (z>0)
        if m.sum() < 3:
            x2, xz = (x*x).sum()+lam, (x*z).sum()
        else:
            x2, xz = (x[m]*x[m]).sum()+lam, (x[m]*z[m]).sum()
...
def main():
    ...
    ap.add_argument("--include", default="s2-eth2,s2-eth1,s1-eth1,s1-eth2,s3-eth5")
    ap.add_argument("--sensors_file", default="")
    ap.add_argument("--links_file", default="links.txt")
    ap.add_argument("--train_busy", type=float, default=0.70)
    ap.add_argument("--val_busy", type=float, default=0.15)
    ap.add_argument("--out", default="dataset_v4.npz")
    args=ap.parse_args()
    ...
    # sensor backlog + lags (sensors only)
    ...
    # temporary plain split to fit AR(1) on train times; will overwrite with busy-aware
    cut1, cut2 = int(0.7*T), int(0.85*T)
    train_idx=np.arange(0,cut1,dtype=np.int64); val_idx=np.arange(cut1,cut2,dtype=np.int64); test_idx=np.arange(cut2,T,dtype=np.int64)

    # busy-aware 70/15/15 by busy times
    y_sum=Y.sum(1); busy=np.where(y_sum>0)[0].astype(int)
    if len(busy)>=10:
        b1=int(round(0.70*len(busy))); b2=int(round(0.85*len(busy)))
        b1=max(1,min(len(busy)-2,b1)); b2=max(b1+1,min(len(busy)-1,b2))
        cut1=busy[b1]; cut2=busy[b2]
    ...
    # ---- AR(1) prior channel: a_i * y_t per node (a_i fit on TRAIN) ----
    a = fit_ar1_per_node(Y, train_idx, lam=1e-6)   # [N]
    AR1 = (Y * a[None,:].astype(np.float32))       # [T,N] predicts approx y_{t+1}
    AR1 = AR1[...,None]                             # [T,N,1]

    # feature tensor
    is_sensor_ch=np.repeat(Smask[:,:,None], T, axis=0).transpose(0,1,2)
    X=np.concatenate([Xr, Sb[...,None], Sb_l1[...,None], Sb_l2[...,None], Sb_l3[...,None],
                      AR1, is_sensor_ch, static], axis=2)

    feat_names = rate_feats + [
        "sensor_backlog","sensor_backlog_lag1","sensor_backlog_lag2","sensor_backlog_lag3",
        "ar1_next","is_sensor","sw_id_z","port_id_z"
    ]

    # normalization (sensor_backlog* over sensors only; others global)
    ...
    for i,f in enumerate(feat_names):
        ...
        if f=="is_sensor":
            ...
            mask=(is_sensor_ch[:,:,0]==1.0); vals=arr[mask]
    ...
    np.savez_compressed(args.out,
        ...
        sensors=np.array(sensors), ar1_coef=a
    )
    print(json.dumps({
