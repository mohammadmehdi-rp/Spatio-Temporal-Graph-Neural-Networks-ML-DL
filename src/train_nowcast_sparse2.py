...
def main():
    ...
    # Zero-inflated fix: include an explicit idle penalty so the model learns
    # to output (near) zero when the queue is empty. Without this, training on
    # busy-only nodes can yield 100% idle false positives.
    ap.add_argument("--idle_w", type=float, default=1.0, help="weight for idle (yt==0) penalty")
    ap.add_argument("--softplus", action="store_true", help="apply softplus to enforce non-negativity")
    ...
    for ep in range(1,args.epochs+1):
        ...
        for t in train_idx.tolist():
            ...
            loss_busy = focal_huber(pred_s[mbusy], targ_s[mbusy], w[mbusy], args.delta) if mbusy.any() else 0.0
            # Idle penalty: push predictions toward 0 when yt==0.
            # Using squared error in scaled domain keeps magnitudes comparable.
            loss_idle = (pred_s[midle]**2).mean() if midle.any() else 0.0
            loss = loss_busy + args.idle_w * loss_idle
    ...
    # also report idle FP on test for quick diagnosis
    idle = (yt==0)
    fp0 = float((yh[idle] > 0).float().mean().item()) if idle.any() else float('nan')
    fp1 = float((yh[idle] > 1).float().mean().item()) if idle.any() else float('nan')
    print(f"TEST nowcast micro {micro:.3f} | macro {macro:.3f} | idleFP>0 {fp0:.3f} | idleFP>1 {fp1:.3f} | saved {args.out}")
