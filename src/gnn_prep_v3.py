...
def main():
    ...
    ap.add_argument("--out", default="dataset_v3.npz")
    args = ap.parse_args()
    test_busy = 1.0 - args.train_busy - args.val_busy
    assert 0.0 < args.train_busy < 1.0 and 0.0 <= args.val_busy < 1.0 and test_busy > 0.0
    ...
    # sensor backlog + lags (sensors only; zeros elsewhere)
    ...
    # ---- busy-aware 70/15/15 split by busy time indices ----
    y_sum = Y.sum(1)
    busy = np.where(y_sum > 0)[0].astype(int)
    if len(busy) >= 10:
        b1 = int(round(args.train_busy * len(busy)))        # e.g., 70%
        b2 = int(round((args.train_busy + args.val_busy) * len(busy)))  # e.g., 85%
        b1 = max(1, min(len(busy)-2, b1))
        b2 = max(b1+1, min(len(busy)-1, b2))
        cut1 = busy[b1]   # last train time idx
        cut2 = busy[b2]   # last val time idx
        ...
        # fallback: plain 80/10/10 by time
        cut1 = int(0.8 * T)
        cut2 = int(0.9 * T)
    ...
    print(json.dumps({
        ...
        "busy_counts": {
            "train": int((Y[train_idx] > 0).sum()),
            "val":   int((Y[val_idx]   > 0).sum()),
            "test":  int((Y[test_idx]  > 0).sum())
        }
    }, indent=2))
